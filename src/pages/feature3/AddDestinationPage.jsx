...
export default function ActivitiesPage() {
  // Get the destination 'id' from the URL parameters using react-router-dom
  const { id } = useParams();
  
  // Find the matching destination from the mock data array, parsing id string to integer
  const dest = destinations.find(d => d.id === parseInt(id));

  // Render a fallback 'Not Found' view if the destination ID doesn't exist
  if (!dest) {
    return (
  ...
  // Helper function to return badge background/text color classes based on difficulty level
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      {/* Page Header section with back button and destination name */}
      <div>
      ...
      {/* Grid container to map and display each available activity */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {dest.activities && dest.activities.length > 0 ? (
          dest.activities.map(activity => (
            ...
            <div key={activity.id} className="bg-white rounded-xl shadow-md p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div className="space-y-2">
                {/* Activity title, duration, and colored difficulty badge */}
              ...
              {/* Activity price display and book now button */}
          ...
          /* Fallback message if no activities are available for the destination */
